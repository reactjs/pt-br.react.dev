...
Definir uma variável de estado enfileirará outra renderização. Mas às vezes você pode querer realizar várias operações sobre o valor antes de enfileirar a próxima renderização. Para fazer isso, é útil entender como o React agrupa atualizações de estado.

</Intro>

<YouWillLearn>

* O que "agrupar" significa e como o React o usa para processar várias atualizações de estado
...
Mas há um outro fator em jogo aqui. **O React espera até que *todo* o código nos manipuladores de eventos tenha sido executado antes de processar suas atualizações de estado.** É por isso que a re-renderização só acontece *após* todas essas chamadas de `setNumber()`.

Isso pode lembrá-lo de um garçom anotando um pedido no restaurante. Um garçom não corre para a cozinha ao mencionar seu primeiro prato! Em vez disso, ele deixa você terminar seu pedido, permitir que você faça alterações e até mesmo anota pedidos de outras pessoas na mesa.

<Illustration src="/images/docs/illustrations/i_react-batching.png"  alt="Um elegante cursor em um restaurante faz um pedido várias vezes com o React, interpretando o papel de garçom. Depois que ela chama setState() várias vezes, o garçom anota o último que ela pediu como seu pedido final." />

Isso permite que você atualize várias variáveis de estado - mesmo de vários componentes - sem acionar muitas [re-renderizações.](/learn/render-and-commit#re-renders-when-state-updates) Mas isso também significa que a UI não será atualizada até _depois_ que seu manipulador de eventos, e qualquer código nele, for concluído. Esse comportamento, também conhecido como **agregação**, faz com que seu aplicativo React funcione muito mais rápido. Também evita lidar com renderizações "meio concluídas" confusas, onde apenas algumas das variáveis foram atualizadas.

**O React não agrega em *múltiplos* eventos intencionais como cliques** - cada clique é tratado separadamente. Fique tranquilo, o React só faz a agregação quando é geralmente seguro fazê-lo. Isso garante que, por exemplo, se o primeiro clique no botão desabilitar um formulário, o segundo clique não o enviará novamente.
...
Aqui, `n => n + 1` é chamada de uma **função atualizadora.** Quando você a passa para um setter de estado:
...
O React armazena `3` como resultado final e o retorna de `useState`.
...
O React armazena `6` como resultado final e o retorna de `useState`. 
...
Aqui está como o React funciona através dessas linhas de código enquanto executa este manipulador de eventos:
...
Então o React armazena `42` como resultado final e o retorna de `useState`.
...
* O React processa atualizações de estado após os manipuladores de eventos terem terminado de executar. Isso se chama agregação.
...
#### Conserte um contador de pedidos {/*fix-a-request-counter*/}

Você está trabalhando em um aplicativo de mercado de arte que permite ao usuário enviar vários pedidos para um item de arte ao mesmo tempo. Cada vez que o usuário pressiona o botão "Comprar", o contador de "Pendentes" deve aumentar em um. Após três segundos, o contador de "Pendentes" deve diminuir, e o contador de "Concluídos" deve aumentar.

No entanto, o contador de "Pendentes" não se comporta como deveria. Quando você pressiona "Comprar", ele diminui para `-1` (o que não deveria ser possível!). E se você clicar rapidamente duas vezes, ambos os contadores parecem se comportar de maneira imprevisível.
...
Dentro do manipulador de eventos `handleClick`, os valores de `pending` e `completed` correspondem ao que eram no momento do evento de clique. Para a primeira renderização, `pending` era `0`, então `setPending(pending - 1)` se torna `setPending(-1)`, o que é errado. Como você quer *incrementar* ou *decrementar* os contadores, em vez de defini-los para um valor concreto determinado durante o clique, você pode passar as funções atualizadoras em vez disso:
...
Role para baixo na pré-visualização do sandbox. Observe que ele mostra **quatro casos de teste.** Eles correspondem aos exemplos que você viu anteriormente nesta página. Sua tarefa é implementar a função `getFinalState` para que ela retorne o resultado correto para cada um desses casos. Se você implementar corretamente, todos os quatro testes devem passar.
...
Se você estiver se sentindo preso, comece com essa estrutura de código:
