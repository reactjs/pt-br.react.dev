...
Estamos felizes em anunciar o lançamento da do React v16.0! Entre as mudanças estão algumas solicitações antigas de recursos, incluindo [**fragmentos**](#new-render-return-types-fragments-and-strings), [**error boundaries**](#better-error-handling), [**portais**](#portals), suporte para [**atributos DOM customizados**](#support-for-custom-dom-attributes), aprimoramento na [**renderização server-side**](#better-server-side-rendering) e [**tamanho de arquivo reduzido**](#reduced-file-size).
...
```js
render() {
  // Não é mais necessário envolver os items da lista em um elemento extra!
  return [
...
Portais fornecem uma maneira first-class de renderizar componentes filhos em um nó do DOM que existe fora da hierarquia dom DOM do componente pai.
...
### Renderização server-side aprimorada {#better-server-side-rendering}
