...
Você pode esperar que essa `<div>` tenha sempre um fundo vermelho, não importa o valor de `toggle`. No entanto, ao alternar o valor de `toggle` entre `true` e `false`, a cor de fundo começa como `red`, então alterna entre `transparent` e `blue`, [como você pode ver nessa demo](https://codesandbox.io/s/blue-water-ghx8mi).
