...
O pacote `react-dom` provê métodos específicos para o DOM que podem ser usado no nível superior de sua aplicação como uma válvula de escape para sair do modelo do React se você precisar. A maioria de seus componentes não deverá precisar deste módulo.
...
> Nota
>
> Nós não suportamos navegadores antigos que não suportam métodos ES5, mas você pode descobrir que suas aplicações funcionam em navegadores antigos se _polyfills_ como [es5-shim and es5-sham](https://github.com/es-shims/es5-shim) forem incluídos na página. Você está por sua conta se optar por seguir esse caminho.
...
>
> Usar `ReactDOM.render()` para fazer hidratar em um contêiner renderizado no servidor está deprecado e será removido no React 17. Ao invés disso, use [`hydrate()`](#hydrate).
...
Remove do DOM um component do React já montado e limpa seus manipuladores de evento e state. Se nenhum component foi montado no contêiner, chamar essa função não faz nada. Retorna `true` se um componente foi desmontado e `false` se não tinha nenhum componente para desmontar.
