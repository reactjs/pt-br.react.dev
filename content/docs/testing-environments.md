...
Os componentes podem estar usando funções baseadas em tempo como `setTimeout`, `setInterval` ou `Date.now`. Em ambientes de teste, pode ser útil simular essas funções com substituições que permitem "avançar" manualmente o tempo. Isso é ótimo para garantir que seus testes sejam executados rapidamente! Os testes que dependem de cronômetros ainda seriam resolvidos em ordem, mas mais rapidamente [<small>(exemplo)</small>](/docs/testing-recipes.html#timers). A maioria dos frameworks, incluindo [Jest](https://jestjs.io/docs/en/timer-mocks), [sinon](https://sinonjs.org/releases/latest/fake-timers) e [lolex]( https://github.com/sinonjs/lolex), permite simular cronômetros em seus testes.
